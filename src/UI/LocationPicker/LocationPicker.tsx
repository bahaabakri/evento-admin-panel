...
const svgHTML = renderToString(
  <IconMapPinFilled size={32} color="var(--primary-color)" />
);
...
export default function LocationPicker({
  ...
  // 🛰️ Automatically get current location on mount ONLY if no defaults
  useEffect(() => {
    if (!defaultLat || !defaultLng) {
      handleGetLocation();
    } else {
      onSelect(defaultLat, defaultLng);
    }
  }, [defaultLat, defaultLng]);
    ...
    <>
      ...
      >
        <CustomButton
          ...
          className={`${classes.currentPosition}`}
        >
          <IconCurrentLocation />
        </CustomButton>
