...
const ImagePicker = ({
  ...
  const onUploadFile = async (
    ...
    if (
      !uploadIntent ||
      Date.now() > new Date(uploadIntent.createdAt).getTime() + 10 * 60 * 1000
    )
      return;
    ...
    if (files && files.length > 0) {
      ...
      for (let i = 0; i < files.length; i++) {
        if (
          ...
          selectedImagesFiles?.some((el) => el.name === files[i].name)
        )
          continue;
  ...
  return (
    <>
      <CustomFormFieldLabel label={label} isRequired={isRequired} />
      <div className={styles["image-picker-wrapper"]}>
        ...
        >
          <div className={styles["image-picker-content"]}>
            {isDragging ? "Drop images here" : "Drag & Drop images or Browse"}
          </div>
          <input
            ref={fileRef}
            type="file"
              ...
              {selectedImages.map((selectedImage) => (
                ...
                >
                  ...
                  <div
                    ...
                    onClick={(event) => deleteImage(event, selectedImage.id)}
                  >
