...
const RolesPage = () => {
    ...
    const fetchAllRoles = async(page:number = 1, perPage:number = 10) => {
        setPage(page)
        const {data:dataRes} = await request<MyResponsePagination<Role>>('get', `admin/roles?page=${page}&perPage=${perPage}`)
        if(dataRes) {
    ...
    const handleDelete = async (row: Role) => {
        try {
            const res = await request('delete', `admin/roles/${row.id}`)
            if (res) {
    ...
    <div>
        ...
        <MainTable 
            ...
            <div className="flex gap-2">
                <ThemeIcon variant="light" color="blue" className="cursor-pointer" size={30} onClick={() => navigateToEditRole(row)}>
        ...
        >
            <CustomButton onClick={navigateToAddRole} leftSection={<IconPlus size={14} />}>
                <div>Add New Role</div>
            </CustomButton>
        </MainTable>
        {
            !loading && roles && roles.length > 0 && 
            <Pagination className="m-auto w-fit" value={activePage} onChange={(page) => fetchAllRoles(page)} total={numOfPages} />
        }
    </div>
